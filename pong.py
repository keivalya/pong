...
def validateentry(*args):
    ...
    if x and y and a and b:
        ...
        C.config(state="disabled", text="Enter Player Names and choose colors")


top = Tk()
# top.overrideredirect(True)
def color(col):
    pc = colorchooser.askcolor()[1]
    col.set(pc)
...
c1.trace("w", validateentry)
c2.trace("w", validateentry)
...
B1 = Button(top, text="Player 1 color", command=lambda: color(c1))
B1.grid(row=0, column=2)
B2 = Button(top, text="Player 2 color", command=lambda: color(c2))
B2.grid(row=1, column=2)
C = Button(top, text="Ok", command=startGame, width=30, state="disabled")
